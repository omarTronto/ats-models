...
def get_all_scores(
    ...
    scores = OrderedDict()
    if 'bleu' in metrics:
        scores['BLEU'] = corpus_bleu(sys_sents, refs_sents, force=True, tokenizer=tokenizer, lowercase=lowercase,
    ...
    if 'samsa' in metrics:
        ...
        scores['SAMSA'] = corpus_samsa(orig_sents, sys_sents, tokenizer=tokenizer, verbose=True, lowercase=lowercase,
                                       tokenizer_obj=tokenizer_obj,)
    if 'fkgl' in metrics:
        scores['FKGL'] = corpus_fkgl(sys_sents, tokenizer=tokenizer, lowercase=lowercase,
                                     tokenizer_obj=tokenizer_obj,)
    if "bertscore" in metrics:
        ...
        (
            ...
            scores["bertscore_f1"],
        ) = corpus_bertscore(sys_sents, refs_sents, tokenizer=tokenizer, lowercase=lowercase, language=language,
                            tokenizer_obj=tokenizer_obj,)
    ...
    if 'fre_corpus' in metrics:
        scores["corpus_FRE"] = corpus_fre(sys_sents, tokenizer=tokenizer, lowercase=lowercase, language=language,
                                           tokenizer_obj=tokenizer_obj,)
    ...
    if 'wiener_sachtextformel_1_corpus' in metrics:
        scores["Wiener-Sachtextformel-1"] = corpus_wiener_1(sys_sents, tokenizer=tokenizer, lowercase=lowercase,
                                                                    language=language,tokenizer_obj=tokenizer_obj,)
    if 'wiener_sachtextformel_2_corpus' in metrics:
        scores["Wiener-Sachtextformel-2"] = corpus_wiener_2(sys_sents, tokenizer=tokenizer, lowercase=lowercase,
                                                                    language=language,tokenizer_obj=tokenizer_obj,)
    if 'wiener_sachtextformel_3_corpus' in metrics:
        scores["Wiener-Sachtextformel-3"] = corpus_wiener_3(sys_sents, tokenizer=tokenizer, lowercase=lowercase,
                                                                    language=language,tokenizer_obj=tokenizer_obj,)
    if 'wiener_sachtextformel_4_corpus' in metrics:
        scores["Wiener-Sachtextformel-4"] = corpus_wiener_4(sys_sents, tokenizer=tokenizer, lowercase=lowercase,
                                                                    language=language,tokenizer_obj=tokenizer_obj,)

    quality_estimation_scores = corpus_quality_estimation(
        orig_sents, sys_sents, tokenizer=tokenizer, lowercase=lowercase, language=language, tokenizer_obj=tokenizer_obj,
    )
    scores = add_dicts(
        scores,
...
def get_score_table_html_single_system(orig_sents, sys_sents, refs_sents, lowercase, tokenizer, metrics, tokenizer_obj):
    return get_score_table_html_multiple_systems(
        orig_sents, [sys_sents], refs_sents, ['System output'], lowercase, tokenizer, metrics, tokenizer_obj
    )


def get_score_table_html_multiple_systems(
    orig_sents, sys_sents_list, refs_sents, system_names, lowercase, tokenizer, metrics, tokenizer_obj,
):
    ...
    sys_sents_list.append(refs_sents[0])
    ...
    with doc.tag('html', lang='en'):
        ...
        with doc.tag('body', klass='container-fluid m-2 mb-5'):
            ...
            with doc.tag('div', klass='container-fluid'):
                ...
                doc.stag('hr')
                doc.asis(get_scores_by_length_html(orig_sents, sys_sents, refs_sents, tokenizer=tokenizer, metrics=metrics,
                                                   lowercase=lowercase, tokenizer_obj=tokenizer_obj))
            doc.line('h2', 'Plots')
            doc.stag('hr')
            with doc.tag('div', klass='container-fluid'):
